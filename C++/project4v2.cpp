        ...
        //Find subnetwork of nodes and edges option
            ...
            if(flag1){
                if(flag2){
                    ...
                    nodes = new char[100];//declare dynamic array to receive input values
                    counter=0;
                    do{
                        cin >> node;
                    ...
                    pos = new int[edge];//records the line number from the input file which contains the corresponding demanded nodes
                    k=0;
                    for(j=0;j<edge;j++){
                        i=0;
                        bool flag=false;
                    ...
                    out.close();//remove original data
                    out.open("subgraph.txt");

                    out << k << endl;//print out number of arcs
                    for(i=0;i<k;i++){//print out arcs
                        out << *(tail+*(pos+i)) << " " << *(head+*(pos+i)) << endl;//write to the file
                    }
